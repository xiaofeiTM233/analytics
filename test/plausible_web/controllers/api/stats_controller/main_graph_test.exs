defmodule PlausibleWeb.Api.StatsController.MainGraphTest do
  ...
  describe "GET /api/stats/main-graph - monetary metrics" do
    setup [:create_user, :log_in, :create_new_site]

    @tag :v2_only
    test "returns a plot of total_revenue for a monetary goal filter", %{
      conn: conn,
      site: site
    } do
      populate_stats(site, [
        build(:event,
          name: "Purchase",
          revenue_reporting_amount: 30.0,
          timestamp: ~N[2021-01-01 00:00:00]
        ),
        build(:event,
          name: "Purchase",
          revenue_reporting_amount: 10.0,
          timestamp: ~N[2021-01-01 00:00:00]
        ),
        build(:event,
          name: "Purchase",
      ...
      conn =
        get(
          conn,
          "/api/stats/#{site.domain}/main-graph?period=custom&from=2021-01-01&to=2021-01-02&interval=date&metric=total_revenue&filters=#{filters}"
        )

      assert %{"plot" => plot} = json_response(conn, 200)

      assert Enum.count(plot) == 2
      assert List.first(plot) == 40.0
      assert List.last(plot) == 10.0
    end

    @tag :v2_only
    test "returns a plot of average_revenue for a monetary goal filter", %{
      conn: conn,
      site: site
    } do
      populate_stats(site, [
      ...
      conn =
        get(
          conn,
          "/api/stats/#{site.domain}/main-graph?period=custom&from=2021-01-01&to=2021-01-02&interval=date&metric=average_revenue&filters=#{filters}"
        )

      assert %{"plot" => plot} = json_response(conn, 200)

      assert Enum.count(plot) == 2
      assert List.first(plot) == 20.0
      assert List.last(plot) == 10.0
    end
  end
end
